...
serve(async (req) => {
  ...
  try {
    ...
    if (password.length < 6) {
      ...
      return new Response(
        JSON.stringify({ error: 'Password must be at least 6 characters long. Please create a stronger password.' }),
        { 
          status: 400, 
          headers: { ...corsHeaders, 'Content-Type': 'application/json' }
        }
    ...
    if (error) {
      ...
      if (error.message.includes('weak_password') || error.message.includes('Password')) {
        errorMessage = 'Password is too weak. Please use a stronger password with at least 6 characters, including letters and numbers.'
